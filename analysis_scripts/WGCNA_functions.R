...
determineSoftPowerWGCNA <- function(data1, outFile, propGenes=1, title="", batch_id=NULL){
  ...
  #print(calc.similarity(data1,batch_id))
  ...
  sft <- pickSoftThreshold.fromSimilarity(calc.similarity(data1,batch_id), powerVector=powers, verbose=5)
...
runWGCNA <- function(data1, propGenes, softPower=10, signedNetwork=TRUE, batch_id=NULL){
  ...
  datExprA1g <- data1
  adjacencyA1 <- adjacency(calc.similarity(data1,batch_id),power=softPower,type=type)#t(datExprA1g)
  diag(adjacencyA1) <- 0
